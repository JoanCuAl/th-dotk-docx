...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:function name="x:get-target" as="element()?">
	...
	<!-- Date : 2022/04/1 -->
	<!-- Origin: Main Book -->
	<!-- Target: allow plugin not to generate links controlled by config file -->
  <xsl:template match="*[contains(@class, ' topic/xref ')]" name="topic.xref">
    ...
    <xsl:choose>
		<xsl:when test="$generate_links = 'false'">
			<!-- not linkin -->
			<xsl:copy-of select="$contents"/>
		</xsl:when>
		<xsl:otherwise>
			<xsl:call-template name="generateExternalLinkFromXref">
  ...
  <xsl:template name="generateExternalLinkFromXref">
	<xsl:param name="contents" as="node()*"/>
    ...
    <xsl:choose>
      <xsl:when test="empty($target)">
		...
		<!-- Date : 2021/05/13 -->
		<!-- Origin: Validation -->
		<!-- Target: Change how links are generated from xrefs to hyperlinks. Generated hyperlinks behave better than xref when generating pdf from word. 
				Inline styles generated using the parent node (to support the term tagging process) -->
      <xsl:when
        test="contains($target/@class, ' topic/topic ') and not(contains($target/@class, ' glossentry/glossentry '))">
        <xsl:variable name="anchor" select="x:bookmark-name($bookmark-prefix.ref, $target)"/>
