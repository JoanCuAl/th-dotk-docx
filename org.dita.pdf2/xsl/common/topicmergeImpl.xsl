...
<xsl:stylesheet version="2.0"
  ...
  <xsl:template match="/">
	...
	<!-- Tradeheader Update -->
	<!-- Document : validation -->
	<!-- Target: Some atributes present in topic ancestors now get inherited by the topic -->
	<!-- Date : 2021/04/29 -->
			...
			<xsl:choose>
				<xsl:when test="@toc">
					<xsl:attribute name="toc">
					  <xsl:value-of select="concat(@toc, $toc)"/>
					</xsl:attribute>
				</xsl:when>
				<xsl:otherwise>
					<xsl:attribute name="toc">
			...
			<xsl:choose>
				<xsl:when test="@print">
					<xsl:attribute name="print">
					  <xsl:value-of select="concat(@print, $print)"/>
					</xsl:attribute>
				</xsl:when>
				<xsl:otherwise>
					<xsl:attribute name="print">
			...
			<xsl:choose>
				<xsl:when test="@outputclass">
					<xsl:attribute name="outputclass">
					  <xsl:value-of select="concat(@outputclass, $outputclass)"/>
					</xsl:attribute>
				</xsl:when>
				<xsl:otherwise>
					<xsl:attribute name="outputclass">
